...
Page({
  ...
  data: {
    ...
    //钻重 
    categoryName: ["30-49", "50-69", "70-99", "100-199", "30以下"],
    stoneWeightType: "",
    //--价格
    priceName: [
  ...
  onLoad: function (options) {
    ...
    wx.getSystemInfo({
      success: function (res) {
        ...
        that.setData({ 
          wheight: _h + 'px', modalHeight: (res.windowHeight - 140) + 'px',
          heightXilie: (_h-300) + 'px'
        })
      }
    })

 
    // wx.createSelectorQuery().select('.shopSwipeBanner').boundingClientRect(function (rect) {
    //   var initRpx = rect.height / 420 ;
    //   var _mdh = parseFloat(that.data.wheight) - parseInt((170 * initRpx + 108 * initRpx + 206 * initRpx));
    //   that.setData({ mdheight: _mdh });
    //   console.log(170 * initRpx)
    //   console.log(108 * initRpx)
    //   console.log(206 * initRpx)
    // }).exec()
    ...
    app.ajax({
      ...
      success: function (res) {
        ...
        for(var i=0;i<data.length;i++){
          if (data[i].placeCode == "applet_clasify_goods_engagement"){
              that.setData({
  ...
  renqiBnt:function(e){
    ...
    var WeightStart = "";
    var WeightEnd = "";
    if (!that.data.largeWeight) {
      that.data.largeWeight = "";
    } else {
      WeightEnd = that.data.largeWeight / 100;
    }
    if (!that.data.smallWeight) {
      that.data.smallWeight = "";
    } else {
      WeightStart = that.data.smallWeight / 100;
    }
    ...
    if(num==1){
      var json = {
        ...
        sort: 1,
        order: 1,
        stoneWeightStart: WeightEnd,
        stoneWeightEnd: WeightStart,
        priceStart: that.data.largePrice,
        priceEnd: that.data.smallPrice,
        series: seriesText,
        stoneWeightType: that.data.stoneWeightType
      }
      app.ajax({
        url: '/goods/list/1/1',
        method: 'GET',
  ...
  //最小钻石重量输入事件
  largeWeight: function (e) {
    this.setData({
      largeWeight: e.detail.value,
      xid:"-1"
    });
  },
  //最大钻石重量输入事件
  smalWeight: function (e) {
    this.setData({
      smallWeight: e.detail.value,
      xid:"-1"
    })
  },
      ...
      success: function (res) {
        if (res.code == "000000") {
          ...
          if (listArray.length > 0) {
            that.setData({
              jiehuiList: that.data.jiehuiList.concat(res.data.rows),
              page: res.data.page
            })
          } else {
            that.setData({
